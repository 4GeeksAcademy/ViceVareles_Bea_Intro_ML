...
# TODO: Check returns on all functions to ensure they are correct and useful
...
def min_max_records(dataframe, target, record_num):
    for x in dataframe.columns:
        ...
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 4))  # Plot with 2 subplots
        ax1.plot(min_values, min_records, 
                 linestyle='-', color='b', label='Occurrences', marker='o') # Min value plots
        ax1.set_xlabel(f'{x}')
...
def outliers_summary(dataset,outliers):  
    #TODO cambiar a datafame por los prints
...
def corr_comparison(dataset,dataset2,dataset3,target):
    ...
    comparison_df = pd.DataFrame({'Original': corr1, 
                                  'Filtered': corr2, 
                                  'Mean rep.': corr3})
